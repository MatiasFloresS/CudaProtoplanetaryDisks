...
__global__ void Substep1Kernel (float *Pressure, float *rho, float *VradInt, float *invdiffRmed, float *pot,
...
__global__ void EvanescentBoundaryKernel(float *Rmed, float *vrad, float *vtheta, float *energy, float *dens, float *AspectRatioRmed,
  float *viscosity_array, float DRMIN, float DRMAX, int nrad, int nsec, float Tin, float Tout, float step, float G, float SIGMASLOPE,
  float FLARINGINDEX, float *GLOBAL_bufarray, float OmegaFrame1, float *SigmaMed, float *EnergyMed, int Adiabaticc, int SelfGravity);
