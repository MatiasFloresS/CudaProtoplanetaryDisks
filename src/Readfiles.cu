...
SIGMASLOPE, RELEASERADIUS, RELEASEDATE, OMEGAFRAME, MASSTAPER, DT, SIGMA0, TRANSITIONRATIO, G, PI,   \
TRANSITIONWIDTH, LAMBDADOUBLING, ADIABATICINDEX, COOLINGTIME0, IMPOSEDDISKDRIFT, FLARINGINDEX,       \
ECCENTRICITY, CAVITYRADIUS, CAVITYRATIO, CAVITYWIDTH, TRANSITIONRADIUS;

extern string DISK, FRAME, OUTERSOURCEMASS, WRITEDENSITY, WRITEVELOCITY, WRITEENERGY, ZMPLUS,        \
WRITETEMPERATURE, WRITEDIVV, WRITEQPLUS, INDIRECTTERM, OUTPUTDIR ,INNERBOUNDARY, LABELADVECTION,     \
TRANSPORT, PLANETCONFIG, SELFGRAVITY, CICPLANET, FORCEDCIRCULAR, ADIABATIC, COOLING, EXCLUDEHILL,    \
RADIALSPACING;

int ExcludeHill, ViscosityAlpha, RocheSmoothing, OpenInner, AdvecteLabel, FastTransport = YES,       \
GuidingCenter = NO, Indirect_Term = YES, IsDisk = YES, NonReflecting = NO, Corotating = NO, LogGrid, \
OuterSourceMass = NO, Evanescent = NO, Write_Density = YES, Write_Velocity = YES, Write_Energy = NO, \
Write_Temperature = NO, Write_DivV = NO, Write_Qplus = NO, SelfGravity = NO, SGZeroMode = NO,        \
ZMPluss = NO, Adiabaticc = NO, Cooling = NO, CICPlanet = NO, ForcedCircular = NO;
